...
<head>
    ...
    <title>Earnings Analytics | WorkPort</title>
    ...
    <style>
        :root {
            --primary: #10b981; /* Emerald Green for Freelancer */
            --primary-lt: #ecfdf5;
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .logo-txt .w { color: var(--slate-900); } .logo-txt .p { color: var(--primary); }
        ...
        .btn-back:hover { border-color: var(--primary); color: var(--primary); background: var(--primary-lt); transform: translateY(-1px); }
        ...
        .status-success { background: var(--primary-lt); color: var(--primary); }
        ...
        .type-up { color: var(--primary); } .type-down { color: var(--danger); }
...
<nav class="nav">
    ...
    <a href="FreelancerHomeServlet" class="btn-back">
...
<div class="container">
    ...
    <%
    // Safely retrieving attributes to prevent Blank White Page (NPE)
    BigDecimal credit = (request.getAttribute("totalCredit") != null) ? (BigDecimal) request.getAttribute("totalCredit") : BigDecimal.ZERO;
    BigDecimal debit = (request.getAttribute("totalDebit") != null) ? (BigDecimal) request.getAttribute("totalDebit") : BigDecimal.ZERO;
    int creditCount = (request.getAttribute("creditCount") != null) ? (Integer) request.getAttribute("creditCount") : 0;
    int debitCount = (request.getAttribute("debitCount") != null) ? (Integer) request.getAttribute("debitCount") : 0;
    ...
    <header class="page-header">
        <div>
            ...
            <p>Track your payouts, service fees, and net income growth.</p>
        </div>
        <button style="background:var(--primary); color:#fff; border:none; padding:12px 24px; border-radius:12px; font-weight:700; cursor:pointer; box-shadow: 0 4px 14px rgba(16,185,129,0.3);">
    ...
    <div class="kpi-grid">
        <div class="kpi-card">
            <div class="kpi-icon" style="background:var(--primary-lt); color:var(--primary);"><i class="bi bi-wallet2"></i></div>
            <span class="kpi-lbl">Available Balance</span>
            <span class="kpi-val">₹ <%= balance %></span>
        </div>
        <div class="kpi-card">
            <div class="kpi-icon" style="background:var(--blue-lt); color:var(--blue);"><i class="bi bi-graph-up-arrow"></i></div>
            <span class="kpi-lbl">Total Earned</span>
            <span class="kpi-val">₹ <%= credit %></span>
        </div>
        <div class="kpi-card">
            <div class="kpi-icon" style="background:var(--danger-lt); color:var(--danger);"><i class="bi bi-send-exclamation"></i></div>
            <span class="kpi-lbl">Total Withdrawn</span>
            <span class="kpi-val">₹ <%= debit %></span>
        </div>
        <div class="kpi-card">
            <div class="kpi-icon" style="background:var(--warn-lt); color:var(--warn);"><i class="bi bi-receipt-cutoff"></i></div>
    ...
    <div class="charts-row">
        <div class="chart-card">
            <div class="chart-title"><i class="bi bi-activity"></i> Monthly Revenue Trend</div>
            <canvas id="lineChart" height="110"></canvas>
        </div>
        <div class="chart-card">
            <div class="chart-title"><i class="bi bi-pie-chart-fill"></i> Income vs Fee Split</div>
    ...
    <div class="table-card">
        <div class="table-header">
            ...
            <div class="search-wrap">
                ...
                <input type="text" placeholder="Search by payment ID...">
            </div>
        </div>
        <table>
            <thead>
            ...
            <tbody>
                ...
                %>
                <tr>
                    <td><span style="font-weight:700; color:var(--primary);">#<%= tx.get("transaction_id") %></span></td>
                    <td style="color:#64748b;">March 31, 2026</td>
                    <td style="font-weight:800;">₹ <%= tx.get("amount") %></td>
                    <td>
                        <% if("CREDIT".equalsIgnoreCase(type)){ %>
                            <span class="type-pill type-up"><i class="bi bi-arrow-down-left-circle-fill"></i> EARNING</span>
                        <% } else { %>
                            <span class="type-pill type-down"><i class="bi bi-arrow-up-right-circle-fill"></i> WITHDRAWAL</span>
                        <% } %>
                    </td>
                    <td>
                        <span class="status <%= "SUCCESS".equalsIgnoreCase(status) ? "status-success" : "status-pending" %>">
...
<script>
    ...
    // Donut Chart: Balance Split
    new Chart(document.getElementById('donutChart'), {
        type: 'doughnut',
        data: {
            labels: ['Income', 'Withdrawals'],
            datasets: [{
                data: [<%= credit %>, <%= debit %>],
        ...
        options: {
            ...
            plugins: { 
                legend: { 
                    position: 'bottom', 
                    labels: { usePointStyle: true, padding: 25, font: { family: 'Inter' } } 
                } 
            }
        }
    });
